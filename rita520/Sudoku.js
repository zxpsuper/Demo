class Sudoku extends Global {
  constructor(options) {
    ...
    this.sudokuItemUnactiveColor =
      options.sudokuItemUnactiveColor || "rgb(255, 255, 255)";
    this.sudokuItemUnactiveTxtColor =
      options.sudokuItemUnactiveTxtColor || "rgb(48, 44, 43)";
    this.sudokuItemUnactiveShadowColor =
      options.sudokuItemUnactiveShadowColor || "rgb(255, 193, 200)";

    this.sudokuItemActiveColor = options.sudokuItemActiveColor || "#ee2746";
    this.sudokuItemActiveTxtColor =
      options.sudokuItemActiveTxtColor || "rgb(255, 255, 255)";
    this.sudokuItemActiveShadowColor =
      options.sudokuItemActiveShadowColor || "rgb(255, 193, 200)";
    ...
    this.SUDOKU_ITEM_MARGIN =
      8 * 3 || this.sudokuSize / this.AWARDS_ROW_LENGTH / 6;
    this.SUDOKU_ITEM_SIZE =
      this.sudokuSize / this.AWARDS_ROW_LENGTH - this.SUDOKU_ITEM_MARGIN;
    this.SUDOKU_ITEM_TXT_SIZE = `bold ${this.SUDOKU_ITEM_SIZE *
      0.12}px Helvetica`;

    this.BUTTON_SIZE =
      this.sudokuSize -
      (this.SUDOKU_ITEM_SIZE * 2 + this.SUDOKU_ITEM_MARGIN * 3);
  ...
  drawSudoku(context) {
    ...
    // 顶点坐标
    let maxPosition =
      this.AWARDS_STEP * this.SUDOKU_ITEM_SIZE +
      this.AWARDS_STEP * this.SUDOKU_ITEM_MARGIN;
    ...
    for (let i = 0; i < this.AWARDS_LEN; i++) {
      ...
      if (i >= this.RIGHT_BOTTOM_POINT && i < this.LEFT_BOTTOM_POINT) {
        let row = Math.abs(this.AWARDS_STEP * 2 - i),
          ...
          x =
            reverseRow * this.SUDOKU_ITEM_SIZE +
            reverseRow * this.SUDOKU_ITEM_MARGIN,
          y = maxPosition;
  ...
  drawSudokuItem(
    ...
    if (content) {
      if (type === "image") {
        ...
        context.translate(
          x +
            this.SUDOKU_ITEM_SIZE / 2 -
            context.measureText(content).width / 2,
  ...
  sudokuItemMove(context) {
    ...
    if (this._jumpingTime >= this._jumpTotalTime) {
      ...
      return;
      let tempPriceJumpIndex =
        this._jumpIndex === 0 ? this.AWARDS_LEN - 1 : this._jumpIndex - 1;
        ...
        canvas.addEventListener(event, e => {
          ...
          console.log(
            ...
            context.isPointInPath(
              loc.x * this.scalingRatio,
              loc.y * this.scalingRatio
            )
          );
